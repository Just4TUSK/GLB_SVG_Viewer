...
Когда пользователь устанавливает повреждение на двумерной схеме автомобиля, вместе с информацией о детали сохраняются нормализованные координаты точки повреждения. Эти координаты представляют собой относительное положение на изображении и находятся в диапазоне от 0 до 1.

После загрузки трёхмерной модели вычисляются её габариты (Bounding Box), которые определяют минимальные и максимальные значения по каждой из трёх осей. На основе этих данных размеры модели переводятся в локальную систему координат.

Далее, в зависимости от выбранного ракурса автомобиля (вид спереди, сзади, слева, справа или сверху), координаты повреждения преобразуются в стартовую точку луча. Для каждого ракурса используется собственная логика расчёта, позволяющая сопоставить координаты на двумерной схеме с соответствующей стороной трёхмерной модели.

После вычисления стартовой точки создаётся луч, направленный внутрь автомобиля. Луч проходит через модель и проверяется на пересечение сначала с предполагаемой повреждённой деталью, а затем, при необходимости, со всей моделью целиком.

Если пересечение найдено, точка пересечения считается реальным местоположением повреждения на поверхности автомобиля. В этой точке создаётся трёхмерный маркер.

Таким образом, координаты из двумерной схемы не используются напрямую для размещения маркера, а служат лишь исходными данными для вычисления точки на поверхности модели посредством проецирования.

Маппинг из 3D в 2D:

При работе с трёхмерной моделью пользователь может установить повреждение непосредственно на поверхности автомобиля.

В момент клика выполняется трассировка луча от камеры к объекту. Система определяет точную точку пересечения луча с геометрией модели и получает её локальные координаты относительно корневого объекта автомобиля.

Затем эти координаты нормализуются относительно габаритов модели. Для каждой оси вычисляется относительное положение точки внутри общего объёма автомобиля.

В результате формируются нормализованные координаты, которые могут быть переданы обратно в Android-приложение и использованы для отображения повреждения на двумерной схеме.

Создание карты (Map) деталей, наименнованиий узлов (мешей):

После загрузки модели выполняется её обход и формируется карта деталей автомобиля. В эту карту заносятся все доступные узлы модели, включая группы и отдельные меши.

При проецировании повреждения система использует название детали, полученное из двумерной схемы, и пытается найти соответствующий объект в карте деталей. Если объект найден, пересечение луча выполняется только с ним.

Если нужная деталь отсутствует или не была найдена, система выполняет поиск пересечения уже по всей модели автомобиля.
